...
This command should generate a file that is called `merge.groups`. Take a look at it with `less`.
...
```
classify.seqs(fasta=merged.fa, group=merge.groups, reference=/usr/local/data/silva_databases/silva.seed_v119.align, taxonomy=/usr/local/data/silva_databases/silva.seed_v119.tax)
```
